...
def run_data_drift_analysis_pipeline(
    ...
    model_op = load_latest_model_from_gcs(
        project=gcs_project, bucket_name=model_bucket, folder_name=model_folder
    )
    cat_op = load_latest_json_from_gcs(
        project=gcs_project, bucket_name=model_bucket, folder_name=model_folder, suffix_type="cat"
    )
...
@pipeline(
    name="concept-drift-analysis-pipeline",
    description="Concept drift detection via performance & score trends",
)
def run_concept_drift_analysis_pipeline(
    ...
    label_op = compute_label_distribution_drift(project=bq_project, date=anchor_date_op.output)
